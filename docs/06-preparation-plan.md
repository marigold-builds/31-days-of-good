...
- [ ] **Project scaffold template repo:** licence, README template, CI (tests on push), issue templates, CONTRIBUTING, SDG badge slot, `fixtures/` convention, release workflow. One for a Python CLI/library, one for a static web tool, one for a Node library or GitHub Action.
- [ ] **Programme site:** static, one page, 31 tiles, daily log, digest archive, disclosure footer. Deployed to GitHub Pages or equivalent.
