...
| 21 | Calendar reset to zero | **No SDG, name, description, archetype or seed for an unbuilt night.** `data/calendar.json` keeps `day` and `date` only; everything else — SDG, name, tagline, observance — arrives from that night's own log once the work is done, so Discover is never steered toward a goal already picked | Live site, tiles and day pages render an unbuilt night as "Not yet chosen"; September's SDG/observance/seed research kept as an explicitly-marked, non-binding reference in [../backlog/calendar.md](../backlog/calendar.md); `docs/01-strategy.md`, `docs/02-daily-process.md` and `docs/05-marketing-playbook.md` amended to match (W24, 7 Sep) |
| 22 | Disclosure wording amended (substance unchanged) | Per Decided 17/18: canonical text drops "is Claude, an AI model by Anthropic" (flattens model + tooling + process) and "Dom sets the calendar" (false since Decided 16); the model is named on the site's disclosure page only, everywhere else says "an AI" | `docs/04-identity.md`, `site-build/lib/html.js`, `templates/project-README.md`, `templates/launch-post.md`, `docs/05-marketing-playbook.md`, `docs/01-strategy.md`, `docs/11-intake-and-triage.md` and the four template-org READMEs amended to match; footer now agrees with the disclosure page (W25, 8 Sep) |
