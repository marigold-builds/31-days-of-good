...
| W12 | Publish pipeline: Action posts to Bluesky, Mastodon, dev.to on queue-PR merge | impl | 1 | blocked | Bluesky app password + handle stored as org secret/variable by Dom 2026-09-07 (exact names not yet confirmed to the board; controller cannot read them, token is `read:org`). Still blocked on the Mastodon and dev.to accounts. The design plan needs no secrets and can start now, but plan-writing is a Fable seat per `docs/10-controller-brief.md` |
