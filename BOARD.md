...
| O17 | **Live finding, for Dom.** Bluesky auth fails `HTTP 400 InvalidEmail - Address local part cannot be empty`; both credentials resolve, so the value is present but `BLUESKY_HANDLE` almost certainly carries a leading `@`. `createSession` wants the bare handle | run 34160699674, 2026-09-07 | Dom edits the org variable; controller cannot (token is `read:org`). Re-run `check-credentials.yml` after |
