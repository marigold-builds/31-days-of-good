...
| O19 | **For Dom, act first.** `BLUESKY_HANDLE` holds the account's email alias, not the handle. Variables are not masked, so `check-credentials.yml`'s mismatch message printed it into a **public** Actions log | run 34160807570 @ a0346b9 | (1) set the variable to `marigoldbuilds.bsky.social`; (2) delete run 34160807570 — controller has not, deletion is irreversible and needs Dom's go; (3) consider rotating the alias |
