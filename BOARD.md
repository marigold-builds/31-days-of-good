...
| W12 | Publish pipeline: Action posts to Bluesky, Mastodon, dev.to on queue-PR merge | impl | 1 | blocked | Bluesky credentials **verified green** 2026-09-07 (run 34160918305, auth-only): `BLUESKY_APP_PASSWORD` org secret, `BLUESKY_HANDLE` org variable = `marigoldbuilds.bsky.social`. Still blocked on the Mastodon and dev.to accounts. Plan being written under W22 |
...
| O21 | **Conflict, for Dom.** The live site presents the 31 fallback *seed* ideas as if they were the plan: `renderIndex`/`renderDay` fall back to `d.name \|\| d.seed`, so every card, day-page `<h1>`, `<title>`, og:title and tile alt text prints a project name for a night nothing has been chosen for. `backlog/calendar.md` calls seeds "not a commitment", and Decided 17 (demand first, `65e0fce`) says the project comes from requests | `site-build/lib/html.js:38,65,78,84` @ 9b6bae8, live | Site built before Decided 17; nobody revisited the fallback. Decided 8 (no "Planned" label) removed the only remaining cue, so it reads as a committed roadmap. Dom's call: SDG/observance/archetype only (controller's recommendation), mark seeds as candidates, or supersede Decided 17. Fixing means regenerating the 31 tiles too |
| O22 | The Bluesky first post is **held** pending O21 — its draft links the calendar, and pointing the account's first post at a page that contradicts the programme's premise is not something to do by default | draft in session, 2026-09-07 | text approved in substance by the controller's read of `docs/04-identity.md`; unblock when O21 is settled |
